...
func (d *SQLiteProxyDAO) List() ([]Proxy, error) {
	rows, err := d.db.Query(`
		SELECT proxy_id, proxy_name, proxy_config, COALESCE(pre_proxy_id, ''), dns_servers, COALESCE(group_name, ''),
...
func (d *SQLiteProxyDAO) Upsert(proxy Proxy) error {
	...
	_, err := d.db.Exec(`
		INSERT INTO browser_proxies (
		  proxy_id, proxy_name, proxy_config, pre_proxy_id, dns_servers, group_name,
		...
		proxy.ProxyId, proxy.ProxyName, proxy.ProxyConfig, proxy.PreProxyId, proxy.DnsServers, proxy.GroupName,
...
func scanProxies(rows *sql.Rows) ([]Proxy, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(
			&p.ProxyId, &p.ProxyName, &p.ProxyConfig, &p.PreProxyId, &p.DnsServers, &p.GroupName,
